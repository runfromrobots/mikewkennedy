...
* `--cds-caption-01` (nav, eyebrow, tags, dates, footer, all the mono/label text) and `--cds-body-compact-01`/`--cds-body-01` (credentials list, a few other spots) were bumped up from Carbon's literal values — 0.75rem to 1rem, and 0.875rem to 1.0625rem respectively — over two rounds, because the smallest text on the page read too small/barely legible, especially set in VT323. Since every one of those elements reads off these tokens, don't hardcode a smaller font-size on top of them for new label-style text — that would silently undo the legibility fix.
