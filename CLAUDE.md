...
* Buttons: flat fill only, no outline/ghost/bordered-transparent style (`.btn-primary` navy, `.btn-secondary` teal-deep). The header "Get in touch" button and the two hero buttons ("See the work" / "Download resume") are oversized per Fitt's Law using a shared `--action-btn-size` token (80px, Carbon's largest button size) — the header button spans the full header bar height flush top/bottom (`.btn-header`, via the `.header-content` wrapper + flex stretch), and the two hero buttons touch with zero gap between them (`.btn-large`, `.hero-actions { gap: 0 }`). Other buttons (contact's "LinkedIn", case-studies' "Back to home") stay normal-sized — only those three specific buttons get the large treatment.
