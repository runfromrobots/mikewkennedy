...
* `--cds-caption-01` (the smallest type-scale token — nav, eyebrow, tags, dates, footer, all the mono/label text) was bumped from Carbon's literal 0.75rem to 0.9375rem site-wide because it read too small/barely legible, especially set in VT323. Since every one of those elements reads off this single token, don't hardcode a smaller font-size on top of it for new label-style text — that would silently undo the legibility fix.
